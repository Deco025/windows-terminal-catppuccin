...
脚本会装齐 pwsh / oh-my-posh / fastfetch / Maple Mono NF / Terminal-Icons，把配置文件放到位（终端设置、PowerShell profile、oh-my-posh 主题、fastfetch 配置和 logo），并关掉 conda 自带的 `(base)` 前缀，否则它会和 oh-my-posh 的 python 段重复显示。profile 的真实路径直接向 pwsh 询问，Documents 被 OneDrive 重定向的机器也能装对位置。覆盖任何已有文件之前都会先备份成 `*.bak-<时间戳>`。
...
**fastfetch logo 颜色**。四色 Windows logo 用的是 Windows 品牌色。由于 fastfetch 内置 logo 的配色槽位在 Windows 版上失效（见下面的坑 #3），颜色是以 ANSI 真彩色序列的形式直接写在 [`fastfetch/win11-logo.txt`](fastfetch/win11-logo.txt) 里的，想换色改这个文件。
...
### fastfetch 内置 logo 的配色不生效（Windows）

给 `windows11` 内置 logo 配 `logo.color`（或用 `--logo-color-1` 等 CLI 参数）在 2.66 到 2.68 的 Windows 版上完全不生效。logo 里的 `$1` 到 `$4` 颜色占位符不会被替换成转义序列，整个 logo 用默认前景色渲染，四色 Windows 标志变成一片灰。模块文字的颜色（键名、色块）都正常，只有 logo 不行。

本仓库的绕过方式是干脆不用 `$N` 占位符，把 ANSI 真彩色序列直接嵌进 logo 文件。[`fastfetch/win11-logo.txt`](fastfetch/win11-logo.txt) 内嵌了四种品牌色（`#F25022` / `#7FBA00` / `#00A4EF` / `#FFB900`），随仓库发布的 `fastfetch/config.jsonc` 以文件 logo 方式加载它：

```jsonc
"logo": { "type": "file", "source": "~/.config/fastfetch/win11-logo.txt" }
```

（`install.ps1` 安装时会把 `~` 改写成绝对路径；手动安装的话，如果 logo 出不来就把它改成绝对路径。）
